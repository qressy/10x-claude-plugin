...
function withStoreId(required: string[], props: Record<string, unknown>) {
  if (!PLATFORM_MODE) return { required, properties: props };
  return {
...
async function listStores(): Promise<string> {
  ...
  const json = await response.json() as { stores: Array<{ store_id: string; name: string; has_checkout: boolean }>; total: number };
  if (json.total === 0) return "No stores are registered on this platform yet.";
...
async function callBackendTool(storeId: string, toolName: string, args: Record<string, unknown>): Promise<string> {
  let response: Response;
  try {
  ...
  if (!response.ok) throw new Error(json.error ?? `Backend returned HTTP ${response.status}`);
  return json.result ?? "";
}
...
const server = new Server(
  { name: "10x-shopping-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "list_stores") {
      ...
      const storeId = a.store_id as string;
      if (!storeId) throw new Error("store_id is required. Use list_stores to find available stores.");
